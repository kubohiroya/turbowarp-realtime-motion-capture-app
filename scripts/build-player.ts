 ...
 */
async function packagePlayer(label: string, sb3: Buffer, target: URL): Promise<boolean> {
  const project = await Packager.loadProject(sb3);
  const extensions = project.analysis.extensions;
  if (extensions.length === 0) {
    failures.push(`${label}: the SB3 embeds no extension, so the app would have no blocks`);
    return false;
  }
...
for (const [app, {lensCalibration}] of Object.entries(config.apps)) {
  ...
  await packagePlayer(app, sb3, new URL(`apps/${app}/dist/${app}-player.html`, repositoryRoot));
   ...
   */
  const calibrationSb3 = await readFile(new URL(lensCalibration.sb3, repositoryRoot)).catch(
    () => null
  );
  ...
  await packagePlayer(
    ...
    calibrationSb3,
    new URL(`apps/${app}/dist/${app}-lens-calibration-player.html`, repositoryRoot)
  );
}
