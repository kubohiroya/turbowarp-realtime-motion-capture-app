...
import {
  cameraAppScripts,
  cameraAppStageData,
} from './app-scripts/camera-app.ts';
import {
  fusionAppScripts,
  fusionAppStageData,
} from './app-scripts/fusion-app.ts';
...
const applications = [
  {
    app: 'camera-app',
    scripts: cameraAppScripts,
    stageData: cameraAppStageData,
  },
  {
    app: 'fusion-app',
    scripts: fusionAppScripts,
    stageData: fusionAppStageData,
  },
...
for (const { app, scripts, ...application } of applications) {
  const projectUrl = new URL(
    `apps/${app}/source/project.source.json`,
    repositoryRoot,
  );
  const project = await readJson<ProjectSource & { targets: StageTarget[] }>(
    projectUrl,
  );
  ...
  if ('stageData' in application) {
    ...
    if ('lists' in application.stageData)
      stage.lists = application.stageData.lists;
  }
...
if (differences.length > 0) {
  ...
  console.error(
    'Run `pnpm run build:scripts` after editing scripts/app-scripts.',
  );
  process.exitCode = 1;
} else {
  console.log(
    `Application scripts match scripts/app-scripts (${applications.length} applications).`,
  );
}
