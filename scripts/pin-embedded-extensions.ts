...
if (declaration.schemaVersion !== 1) {
  throw new Error(
    `Unsupported app-extensions schemaVersion: ${declaration.schemaVersion}`,
  );
}
...
function packageDirectory(
  application: string,
  extension: AppExtensionEntry,
): URL {
  if (extension.provider === 'workspace') {
    if (extension.directory === undefined) {
      throw new Error(
        `${extension.id} is a workspace extension without a directory.`,
      );
    }
  ...
  if (extension.provider === 'npm') {
    return new URL(
      `apps/${application}/node_modules/${extension.package}/`,
      repositoryRoot,
    );
  }
  throw new Error(
    `Unknown provider for ${extension.id}: ${String(extension.provider)}`,
  );
}
...
async function materialize(
  ...
  if (tracked && !writeTracked) {
    differences.push(
      existing === null ? `${path} is missing` : `${path} is out of date`,
    );
    return;
  }
...
for (const { app, bundle, extensions } of declaration.apps) {
  ...
  if (extensions[0]?.provider !== 'workspace') {
    throw new Error(
      `${app} must declare its app shell as the first bundle member.`,
    );
  }
  ...
  for (const extension of extensions) {
    ...
    if (!/^\d+\.\d+\.\d+$/.test(manifest.version)) {
      throw new Error(
        `${extension.package} is not pinned to an exact version: ${manifest.version}`,
      );
    }

    const javascript = await readFile(new URL(extension.artifact, directory));
    await materialize(
      new URL(`${extension.id}.js`, extensionsDirectory),
      javascript,
      {
        tracked: false,
      },
    ...
    if (extension.provider === 'npm') {
      ...
      if (extension.apiManifest !== undefined) {
        const apiManifest = await readFile(
          new URL(extension.apiManifest, directory),
        );
        const parsed = JSON.parse(
          apiManifest.toString('utf8'),
        ) as ExtensionManifest;
        if (parsed.id !== extension.id) {
          throw new Error(
            `${extension.package} publishes extension ID ${parsed.id}`,
          );
        }
        await materialize(
...
if (differences.length > 0) {
  ...
  console.error(
    'Run `pnpm run pin:extensions` after building packages/app-shell.',
  );
  process.exitCode = 1;
} else {
  console.log(
