...
describe('applyFeatureFlags', () => {
  ...
  it('writes both time-space-sync flags from the application setting, off unless asked', () => {
    ...
    expect(on[timeSpaceSyncFlagGlobalKey]).toEqual({
      opticalTimeSyncV1: true,
      placementSolveV1: true,
    });
    expect(off[timeSpaceSyncFlagGlobalKey]).toEqual({
      opticalTimeSyncV1: false,
      placementSolveV1: false,
    });
  });
