...
function meter() {
  ...
  return {clock, meter: new PoseMeter({nowMs: () => clock.nowMs, pageTimeUs: () => clock.pageTimeUs})};
}

const status = (fields: Record<string, unknown>) =>
  JSON.stringify({state: 'ready', error: '', frameTimeSource: 'capture', persons: 1, ...fields});
...
describe('PoseMeter', () => {
  it('measures each camera over one-second windows, and the round and capture spread across cameras', () => {
    ...
    for (let round = 0; round <= 20; round += 1) {
      ...
      m.record('cam-1', status({inferences: round, skippedFrames: 0, lastInferenceMs: 20.04, captureTimestampUs: clock.pageTimeUs - 60_000, persons: 2}));
      m.record('cam-2', status({inferences: round, skippedFrames: round, lastInferenceMs: 18, captureTimestampUs: clock.pageTimeUs - 45_000}));
      m.endCycle();
    }
    ...
    expect(measurement.cameras).toEqual([
      {cameraId: 'cam-1', state: 'ready', error: '', inferenceMs: 20, inferenceFps: 20, skippedFps: 0, captureToResultMs: 60, frameTimeSource: 'capture', persons: 2},
      {cameraId: 'cam-2', state: 'ready', error: '', inferenceMs: 18, inferenceFps: 20, skippedFps: 20, captureToResultMs: 45, frameTimeSource: 'capture', persons: 1}
    ]);
  ...
  it('keeps the delay of a frame until a new one is inferred, and names a presentation time', () => {
    const {clock, meter: m} = meter();
    m.record('cam-1', status({inferences: 1, captureTimestampUs: 999_900_000, frameTimeSource: 'presentation'}));
  ...
  it('ignores text that is not a status, reports errors, and forgets on reset', () => {
    ...
    m.record('cam-1', '[1]');
    expect(m.summary()).toBe('');
    m.record('cam-1', status({state: 'error', error: 'inference-failed: lost device'}));
    expect(m.summary()).toContain('cam-1: エラー inference-failed: lost device');
    m.reset();
