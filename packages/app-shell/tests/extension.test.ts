...
function createExtension() {
  ...
  const flags = {
    flags: resolveFeatureFlags(cameraAppConfig.featureFlags),
    state: 'applied',
  } as const;
  ...
  return {
    calls,
    extension: new MultiviewPoseAppShellExtension(
      cameraAppConfig,
      recording,
      flags,
      {
...
describe('getInfo', () => {
  ...
  it('leaves the title, menu, and DSL surface to turbowarp-title-menu', () => {
    ...
    for (const absent of [
      ...
      'setAppStatus',
      'setAppMenuActionEnabled',
    ]) {
      expect(opcodes).not.toContain(absent);
    }
    expect(Object.keys(info['menus'] as Record<string, unknown>)).toEqual([
      'featureFlags',
    ]);
  });
  ...
  it('offers the lens calibration blocks only to an application given a launcher', () => {
    ...
    const fusion = new MultiviewPoseAppShellExtension(
      fusionAppConfig,
      fusionShell,
      {
    ...
    const fusionOpcodes = (
      fusion.getInfo()['blocks'] as Array<Record<string, unknown>>
    ).map((block) => block['opcode']);
    expect(fusionOpcodes).toHaveLength(31);
    ...
    expect(cameraOpcodes).toEqual(
      expect.arrayContaining([
        ...
        'openLensCalibrationAppForCamera',
...
describe('diagnostic reporters', () => {
  ...
  it('reports an application flag only when the application lists it', () => {
    const { extension } = createExtension();
    expect(
      extension.appFeatureEnabled({ FEATURE: 'external3dServiceV1' }),
    ).toBe(false);
    ...
    expect(enabled.appFeatureEnabled({ FEATURE: 'external3dServiceV1' })).toBe(
      true,
    );
  });
  ...
  it('reports only the flags this application requested', () => {
    const { extension } = createExtension();
    expect(
      extension.appFeatureEnabled({ FEATURE: 'cameraCalibrationV1' }),
    ).toBe(true);
    expect(extension.appFeatureEnabled({ FEATURE: 'poseFusion3D' })).toBe(
      false,
    );
