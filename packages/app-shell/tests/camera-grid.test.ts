...
import {
  CameraGrid,
  constraintsFor,
  type CameraSourcePort,
} from '../src/camera-grid.js';
...
function fakeSource(options: { refuse?: string } = {}) {
  ...
  const source: CameraSourcePort = {
    async acquireCamera(request) {
      ...
      const ideal = (value: unknown) =>
        (value as { ideal?: number } | undefined)?.ideal ?? 0;
      const track: FakeTrack = {
        label: `USB Camera (${request.cameraId})`,
        readyState: 'live',
        getSettings: () => ({
          width: ideal(request.video?.width),
          height: ideal(request.video?.height),
          frameRate: 30,
          deviceId: `device-${request.cameraId}`,
        }),
      };
...
const request = (
  ...
  height = 720,
  frameRate = 30,
) => ({ cameraId, deviceId, width, height, frameRate });
...
describe('CameraGrid', () => {
  ...
  it('restarts a camera ID with a new request and releases the old lease first', async () => {
    ...
    expect(
      acquired.map((entry) => (entry.video?.width as { ideal: number }).ideal),
    ).toEqual([640, 1280]);
  });
  ...
  it('keeps the browser error when a camera cannot start', async () => {
    ...
    expect(cameras.report('cam-2')).toMatchObject({
      state: 'error',
  ...
  it('reports cameras in ID order and forgets stopped ones', async () => {
    ...
    await cameras.start(request('cam-2', 'usb-b'));
    await cameras.start(request('cam-1', 'usb-a'));
    expect(cameras.reports().map((report) => report.cameraId)).toEqual([
      'cam-1',
      'cam-2',
    ]);
  ...
  it('keeps the latest pose frame per running camera and clears it for text that is not one', async () => {
    ...
    const frame = {
      ...
      persons: [
        {
          keypoints: [
            { id: 'nose', x: 10, y: 20, score: 0.9 },
            { id: 'broken', x: 'left', y: 1 },
          ],
        },
      ],
    };
