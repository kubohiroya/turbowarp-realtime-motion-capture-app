...
function envelope(type: string, peer: string, payload: unknown, channel = 'default'): string {
  return JSON.stringify({version: 1, id: `${type}-${Math.random()}`, seq: 1, from: 'x', channel, type, payload, timestamp: 0, peer});
}
...
function setup(messages: string[], now = {value: 1000}) {
  ...
  let loaded = true;
  const router = new NetworkRouter({webrtc: () => (loaded ? queue : null), nowMs: () => now.value});
...
describe('NetworkRouter', () => {
  ...
  it('keeps every other message in arrival order', () => {
    ...
    const {router} = setup([first, envelope('latest-data', 'camera-1', {}, 'pose'), second]);
    router.pump();
  ...
  it('bounds the control queue and counts what it had to drop', () => {
    const messages = Array.from({length: 205}, (_, index) => envelope('event', 'fusion', {index}));
...
describe('createScratchNetworkRouterHost', () => {
  it('finds the WebRTC queue blocks inside a static bundle and on their own', async () => {
    const {createScratchNetworkRouterHost} = await import('../src/network-router.js');
    const scratch = (globalThis as {Scratch?: {vm?: {runtime?: Record<string, unknown>}}}).Scratch ?? {};
    (globalThis as Record<string, unknown>)['Scratch'] = scratch;
    const messages = ['a', 'b'];
    for (const prefix of ['realtimemotioncapturefusionapp_kubohiroyawebrtc__', 'kubohiroyawebrtc_']) {
