...
const exported = readFileSync(
  new URL('./fixtures/lens-calibration-profile.yaml', import.meta.url),
  'utf8',
);

const require = createRequire(import.meta.url);
const bundle = readFileSync(
  require.resolve('@kubohiroya/turbowarp-camera-source/camera-source.js'),
  'utf8',
);
...
function loadCameraSource(): CameraSourceBlocks {
  ...
  const scratch = {
    ...
    extensions: {
      unsandboxed: true,
      register: (extension: unknown) => (registered = extension),
    },
    BlockType: {
      COMMAND: 'command',
      REPORTER: 'reporter',
...
async function loadLensCalibrationFile(
  camera: CameraSourceBlocks,
  text: string,
): Promise<string> {
  camera.registerCameraProfileAs({ PROFILE_JSON: text, CAMERA_ID: 'pose' });
  if (camera.cameraProfileError() !== '') return 'invalid';
  if (camera.cameraProfileCompatibility({ CAMERA_ID: 'pose' }) === 'compatible')
    return 'used';
...
describe('the lens calibration file', () => {
  ...
  it('is used on the camera it was calibrated on', async () => {
    ...
    expect(
      JSON.parse(camera.cameraProfileJson({ CAMERA_ID: 'pose' })).cameraId,
    ).toBe('pose');
  });
  ...
  it('is refused, with the member named, when the text is not a calibration', async () => {
    ...
    const camera = loadCameraSource();
    await camera.startSharedCamera({ CAMERA_ID: 'pose' });
    const broken = exported.replace(
      'rows: 3\n  cols: 3\n  data: [912.4371',
      'rows: 3\n  cols: 3\n  data: [0',
    );
    expect(await loadLensCalibrationFile(camera, broken)).toBe('invalid');
    expect(camera.cameraProfileErrorDetail()).toMatch(
      /intrinsics\.fx|camera_matrix/,
    );
  });
});
