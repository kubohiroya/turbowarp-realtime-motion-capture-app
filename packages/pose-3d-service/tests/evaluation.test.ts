...
const scene = (overrides = {}) =>
  generateScene({ seconds: 2, cameras: 3, persons: 2, ...overrides });
    ...
    handle(message: unknown) {
      ...
      if (envelope.type === 'configure') {
        return response(envelope.id, 'configured', {
          cameraIds: (envelope.payload as ServiceConfiguration).cameras.map(
      ...
      if (envelope.type === 'frame2d') {
        const payload = envelope.payload as {
          cameraId: string;
          frame: { captureTimestampUs: number; sequence: number };
        };
        latestTimestampUs = Math.max(
          latestTimestampUs,
          payload.frame.captureTimestampUs,
        );
        return response(envelope.id, 'accepted', {
          cameraId: payload.cameraId,
          sequence: payload.frame.sequence,
        });
      }
      const truthFrame = truth.find(
        (candidate) => candidate.timestampUs === latestTimestampUs,
      );
...
describe('synthetic scenes', () => {
  it('produces valid 2D frames whose cameras and calibrations match the configuration', () => {
    const { session } = scene();
    const cameraIds = session.configuration.cameras.map(
      (camera) => camera.cameraId,
    );
    ...
    for (const event of frames) {
      ...
      const camera = session.configuration.cameras.find(
        (candidate) => candidate.cameraId === event.cameraId,
      );
  ...
  it('records which cameras saw each truth joint, and most are seen by two or more', () => {
    const { session } = scene();
    const joints = (session.truth ?? []).flatMap((frame) =>
      frame.persons.flatMap((person) => person.joints),
    );
    expect(joints.length).toBeGreaterThan(1000);
    const triangulable = joints.filter(
      (joint) => joint.cameraIds.length >= 2,
    ).length;
    expect(triangulable / joints.length).toBeGreaterThan(0.9);
    for (const joint of joints) {
      for (const cameraId of joint.cameraIds)
        expect(['camera-1', 'camera-2', 'camera-3']).toContain(cameraId);
    }
  });

  it('is the same scene for the same seed and a different one for another', () => {
    expect(serializeSession(scene().session)).toBe(
      serializeSession(scene().session),
    );
    expect(serializeSession(scene({ seed: 2 }).session)).not.toBe(
      serializeSession(scene().session),
    );
  });
  ...
  it('loses people for spells and gives them new tracking IDs, which the truth does not', () => {
    const { session } = scene({
      occlusionRate: 2,
      identitySwitchRate: 2,
      seed: 7,
    });
    const counts = session.events.flatMap((event) =>
      event.type === 'frame2d' ? [event.frame.persons.length] : [],
    );
    expect(Math.min(...counts)).toBeLessThan(2);
    const trackingIds = new Set(
      session.events.flatMap((event) =>
        event.type === 'frame2d'
          ? event.frame.persons.map((person) => person.trackingId)
          : [],
      ),
    );
    expect(trackingIds.size).toBeGreaterThan(2);
    const truthIds = new Set(
      (session.truth ?? []).flatMap((frame) =>
        frame.persons.map((person) => person.personId),
      ),
    );
...
describe('replaying a session', () => {
      ...
      events: [
        {
          ...
          atUs: 1,
          cameraId: 'camera-1',
          frame: poseFrame('peer', 'wrong-calibration', 1),
        },
      ],
    };
    const replay = replaySession(session, { nowMs: counter() });
  ...
  it('stops when the configuration is refused', () => {
    const broken: Session = {
      ...scene().session,
      configuration: { ...configuration(), cameras: [] },
    };
...
describe('metrics', () => {
  it('shows no error for a service that answers with the truth', () => {
    ...
    const metrics = evaluate(
      session,
      replaySession(session, { service: oracle(session), nowMs: counter() }),
    );
  ...
  it('measures how far a wrong answer is, in meters', () => {
    ...
    const metrics = evaluate(
      ...
      replaySession(session, {
        service: oracle(session, { shiftMeters: 0.25 }),
        nowMs: counter(),
      }),
    );
    ...
    const stub = evaluate(
      session,
      replaySession(session, { nowMs: counter() }),
    );
  ...
  it('counts an identity switch when the same person is reported under another ID', () => {
    ...
    const swapping: ServiceHandler = {
      handle(message: unknown) {
        const answer = truthful.handle(message) as {
          type: string;
          payload: PoseFrame3DV2 | null;
        };
        ...
        return {
          ...
          payload: {
            ...answer.payload,
            persons: answer.payload.persons.map((person) => ({
              ...person,
              personId: `${person.personId}-again`,
            })),
          },
        };
      },
    ...
    const metrics = evaluate(
      session,
      replaySession(session, { service: swapping, nowMs: counter() }),
    );
  ...
  it('reports what it can for a session with no truth, and says so by leaving truth out', () => {
    ...
    const metrics = evaluate(
      withoutTruth,
      replaySession(withoutTruth, { nowMs: counter() }),
    );
...
describe('sessions', () => {
  ...
  it('refuses what it cannot replay', () => {
    const reasons = [
      ...
      parseSession(
        JSON.stringify({ schema: 'twrmc/pose-3d-session', version: 2 }),
      ),
      parseSession(
        JSON.stringify({
          schema: 'twrmc/pose-3d-session',
...
it('uses the COCO-17 joints', () => {
  const { session } = scene();
  const joints = session.truth?.[0]?.persons[0]?.joints.map(
    (joint) => joint.id,
  );
