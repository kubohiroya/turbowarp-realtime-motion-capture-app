  ...
  const port: ServicePort = {
    post: (message) => {
      ...
      queueMicrotask(() => {
        ...
        if (reply !== null)
          listener(options.mutate ? options.mutate(reply) : reply);
      });
    },
    onMessage: (next) => {
      listener = next;
...
async function configured(
  implementation: Parameters<typeof configuration>[0] = 'stub-normal',
  mutate?: (message: unknown) => unknown,
) {
...
const send = (client: Pose3dServiceClient, sequence: number, age = 10) => {
  client.sendFrame(
    'camera-1',
    JSON.stringify(frame('camera-1', 'cal-1', sequence)),
    age,
  );
  client.sendFrame(
    'camera-2',
    JSON.stringify(frame('camera-2', 'cal-2', sequence)),
    age,
  );
};
...
describe('Pose3dServiceClient', () => {
  ...
  it('forwards the capture timestamp unchanged', async () => {
    ...
    await flush();
    await client.requestPose3d();
    expect(client.latestFrame()?.timestampUs).toBe(
      frame('camera-1', 'cal-1', 7).captureTimestampUs,
    );
  });
  ...
  it('does not forward a repeat, a stale frame, an invalid frame or another calibration', async () => {
    ...
    client.sendFrame(
      'camera-1',
      JSON.stringify(frame('camera-1', 'cal-1', 2)),
      LIMITS.maxFrameAgeMs + 1,
    );
    client.sendFrame('camera-1', '{"schema":"twrmc/pose-frame-2d"}', 0);
    client.sendFrame(
      'camera-2',
      JSON.stringify(frame('camera-2', 'cal-1', 3)),
      0,
    );
  ...
  it('clears the last good frame once a later answer is invalid', async () => {
    ...
    const { client } = await configured('stub-normal', (reply) => {
      const message = reply as {
        type: string;
        payload: { persons?: unknown[] } | null;
      };
      return corrupt && message.type === 'pose3d' && message.payload
        ? { ...message, payload: { ...message.payload, persons: 'none' } }
        : reply;
    });
    send(client, 1);
    await flush();
