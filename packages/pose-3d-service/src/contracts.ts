...
export const JOINT_STATES: readonly JointState[] = ['measured', 'constrained', 'predicted', 'missing'];
 ...
 */
export type ImplementationId = 'fusion-v0' | 'stub-normal' | 'stub-timeout' | 'stub-invalid';
export const IMPLEMENTATIONS: readonly ImplementationId[] = ['fusion-v0', 'stub-normal', 'stub-timeout', 'stub-invalid'];
...
export type ServiceRequest =
  ...
  | Envelope<'frame2d', {readonly cameraId: string; readonly frame: PoseFrame2D}>
  | Envelope<'requestPose3d', {readonly timestampUs: number | null}>;

export type ServiceResponse =
  | Envelope<'configured', {readonly cameraIds: readonly string[]}>
  | Envelope<'accepted', {readonly cameraId: string; readonly sequence: number}>
  | Envelope<'pose3d', PoseFrame3DV2 | null>
  | Envelope<'error', {readonly code: ServiceErrorCode; readonly message: string}>;
...
export function request<Type extends ServiceRequest['type']>(
  ...
  return {interface: SERVICE_INTERFACE, version: SERVICE_INTERFACE_VERSION, id, type, payload} as ServiceRequest;
}
...
export function response<Type extends ServiceResponse['type']>(
  ...
  return {interface: SERVICE_INTERFACE, version: SERVICE_INTERFACE_VERSION, id, type, payload} as ServiceResponse;
}
...
const fail = <T>(code: ServiceErrorCode, message: string): Validation<T> => ({ok: false, code, message});
...
export function validatePoseFrame2D(value: unknown): Validation<PoseFrame2D> {
  if (!isObject(value)) return fail('invalid-payload', 'A pose frame must be an object.');
  if (value['schema'] !== POSE_FRAME_2D_SCHEMA) {
    return fail('invalid-payload', `A 2D pose frame must have schema ${POSE_FRAME_2D_SCHEMA}.`);
  }
  if (value['version'] !== 1 && value['version'] !== 2) {
    return fail('unsupported-version', `2D pose frame version ${String(value['version'])} is not supported.`);
  }
  for (const key of ['cameraId', 'peerId', 'calibrationId'] as const) {
    if (!isIdentifier(value[key])) return fail('invalid-payload', `${key} must be an identifier.`);
  }
  if (!isTimestamp(value['sequence']) || !isTimestamp(value['captureTimestampUs'])) {
    return fail('invalid-payload', 'sequence and captureTimestampUs must be non-negative integers.');
  }
  for (const key of ['frameWidth', 'frameHeight'] as const) {
    const size = value[key];
    if (!isFiniteNumber(size) || size < 1) return fail('invalid-payload', `${key} must be positive.`);
  }
  const persons = value['persons'];
  if (!Array.isArray(persons) || persons.length > LIMITS.maxPersons) {
    return fail('invalid-payload', `persons must be an array of at most ${LIMITS.maxPersons}.`);
  }
  for (const person of persons) {
    if (!isObject(person) || typeof person['trackingId'] !== 'string' || !isScore(person['score'])) {
      return fail('invalid-payload', 'Each person needs a trackingId and a score.');
    }
    const keypoints = person['keypoints'];
    if (!Array.isArray(keypoints) || keypoints.length !== COCO_17_KEYPOINT_IDS.length) {
      return fail('invalid-payload', 'Each person needs the 17 COCO keypoints.');
    }
    for (const [index, keypoint] of keypoints.entries()) {
      if (
        ...
        return fail('invalid-payload', `Keypoint ${index} is not a finite ${COCO_17_KEYPOINT_IDS[index]}.`);
      }
    }
  }
...
function validateCamera(value: unknown): Validation<ServiceCamera> {
  ...
  if (!isObject(model) || !isObject(model['intrinsics']) || !isObject(model['distortion'])) {
    return fail('invalid-payload', `Camera ${value['cameraId']} needs a camera model with distortion.`);
  }
  const intrinsics = model['intrinsics'];
  for (const key of ['fx', 'fy', 'cx', 'cy', 'skew']) {
    if (!isFiniteNumber(intrinsics[key])) {
      return fail('invalid-payload', `Camera ${value['cameraId']} intrinsics.${key} must be finite.`);
    }
  }
  if (!isIdentifier(model['intrinsicProfileId'])) {
    return fail('invalid-payload', `Camera ${value['cameraId']} needs intrinsicProfileId.`);
  }
  const transform = value['cameraFromReference'];
  if (!Array.isArray(transform) || transform.length !== 16 || !transform.every(isFiniteNumber)) {
    return fail('invalid-payload', `Camera ${value['cameraId']} cameraFromReference must be 16 finite numbers.`);
  }
  return {ok: true, value: value as unknown as ServiceCamera};
}

export function validateConfiguration(value: unknown): Validation<ServiceConfiguration> {
  if (!isObject(value)) return fail('invalid-payload', 'A configuration must be an object.');
  if (!IMPLEMENTATIONS.includes(value['implementation'] as ImplementationId)) {
    return fail('invalid-payload', `Unknown implementation ${String(value['implementation'])}.`);
  }
  if (!isIdentifier(value['referenceId'])) return fail('invalid-payload', 'referenceId must be an identifier.');
  const cameras = value['cameras'];
  if (!Array.isArray(cameras) || cameras.length < 2 || cameras.length > LIMITS.maxCameras) {
    return fail('invalid-payload', `A configuration needs 2 to ${LIMITS.maxCameras} cameras.`);
  }
  ...
  for (const camera of cameras) {
    ...
    if (seen.has(checked.value.cameraId)) {
      return fail('invalid-payload', `Camera ${checked.value.cameraId} appears twice.`);
    }
...
export function validatePoseFrame3D(value: unknown): Validation<PoseFrame3DV2> {
  if (!isObject(value)) return fail('invalid-response', 'A 3D pose frame must be an object.');
  if (value['schema'] !== POSE_FRAME_3D_SCHEMA || value['version'] !== POSE_FRAME_3D_VERSION) {
    return fail('invalid-response', `A 3D pose frame must be ${POSE_FRAME_3D_SCHEMA} version ${POSE_FRAME_3D_VERSION}.`);
  }
  if (!isTimestamp(value['sequence']) || !isTimestamp(value['timestampUs'])) {
    return fail('invalid-response', 'sequence and timestampUs must be non-negative integers.');
  }
  if (!isIdentifier(value['referenceId'])) return fail('invalid-response', 'referenceId must be an identifier.');
  if (!IMPLEMENTATIONS.includes(value['implementation'] as ImplementationId)) {
    return fail('invalid-response', 'implementation must name a known implementation.');
  }
  const persons = value['persons'];
  if (!Array.isArray(persons) || persons.length > LIMITS.maxPersons) {
    return fail('invalid-response', `persons must be an array of at most ${LIMITS.maxPersons}.`);
  }
  const personIds = new Set<string>();
  for (const person of persons) {
    if (!isObject(person) || !isIdentifier(person['personId']) || !isScore(person['confidence'])) {
      return fail('invalid-response', 'Each person needs a personId and a confidence.');
    }
    if (personIds.has(person['personId'])) {
      return fail('invalid-response', `Person ${person['personId']} appears twice.`);
    }
    personIds.add(person['personId']);
    const error = person['meanReprojectionErrorPx'];
    if (error !== null && !(isFiniteNumber(error) && error >= 0)) {
      return fail('invalid-response', 'meanReprojectionErrorPx must be null or non-negative.');
    }
    const joints = person['joints'];
    if (!Array.isArray(joints) || joints.length !== COCO_17_KEYPOINT_IDS.length) {
    ...
    for (const [index, joint] of joints.entries()) {
      ...
      for (const key of ['x', 'y', 'z', 'sigma'] as const) {
        ...
        if (missing ? coordinate !== 0 : !isFiniteNumber(coordinate)) {
          return fail('invalid-response', `Joint ${String(joint['id'])} ${key} is invalid for state ${String(joint['state'])}.`);
        }
      }
      if (!missing && (joint['sigma'] as number) < 0) {
        return fail('invalid-response', `Joint ${String(joint['id'])} sigma must not be negative.`);
      }
      if (joint['state'] === 'measured' && (joint['cameraIds'] as unknown[]).length < 2) {
        return fail('invalid-response', `Joint ${String(joint['id'])} cannot be measured by fewer than two cameras.`);
      }
    }
  }
...
export function validateResponse(value: unknown): Validation<ServiceResponse> {
  if (!isObject(value) || value['interface'] !== SERVICE_INTERFACE) {
    return fail('unsupported-interface', 'The response does not name the pose 3D service interface.');
  }
  if (value['version'] !== SERVICE_INTERFACE_VERSION) {
    return fail('unsupported-version', `Interface version ${String(value['version'])} is not supported.`);
  }
  if (!Number.isSafeInteger(value['id'])) return fail('invalid-response', 'The response has no id.');
  if (serializedBytes(value) > LIMITS.maxMessageBytes) {
    return fail('payload-too-large', 'The response exceeds the message size limit.');
  }
  switch (value['type']) {
    case 'configured':
    case 'accepted':
    ...
    case 'pose3d': {
      if (value['payload'] === null) return {ok: true, value: value as unknown as ServiceResponse};
      const pose = validatePoseFrame3D(value['payload']);
      return pose.ok ? {ok: true, value: value as unknown as ServiceResponse} : pose;
    }
    case 'error': {
    ...
    default:
      return fail('invalid-response', `Unknown response type ${String(value['type'])}.`);
  }
}
...
export function toPoseFrame3DV1(frame: PoseFrame3DV2): unknown {
  const persons = frame.persons.flatMap((person) => {
    const cameraIds = [...new Set(person.joints.flatMap((joint) => joint.cameraIds))].sort();
    ...
    return [
      {
        ...
        keypoints: person.joints.map((joint) => ({
          ...
          y: joint.y,
          z: joint.z,
          score: joint.state === 'measured' || joint.state === 'constrained' ? person.confidence : 0
        }))
      }
    ];
  });
  return {schema: POSE_FRAME_3D_SCHEMA, version: 1, sequence: frame.sequence, timestampUs: frame.timestampUs, persons};
}
