...
async function start(overrides: Partial<Parameters<typeof startLocalHost>[0]> & {withoutStore?: boolean} = {}) {
...
describe('recording names', () => {
  it('accepts a plain name and refuses anything that could leave the directory', () => {
    ...
    for (const name of ['../escape.json', 'sub/walk.json', '.hidden.json', 'walk.txt', 'walk.json/', `${'a'.repeat(80)}.json`]) {
...
describe('the recordings route', () => {
  it('writes, lists, reads back and removes a recording', async () => {
    ...
    const session = JSON.stringify({schema: 'twrmc/pose-3d-session', version: 1, events: []});

    const written = await fetch(route(host, '&name=walk-1.json'), {method: 'PUT', body: session});
    expect(written.status).toBe(200);
    expect(await readFile(join(recordingsDirectory, 'walk-1.json'), 'utf8')).toBe(session);

    const listed = await fetch(route(host));
    expect(listed.status).toBe(200);
    const body = (await listed.json()) as {recordings: Array<{name: string; bytes: number; modifiedAt: string}>};
    expect(body.recordings.map((entry) => entry.name)).toEqual(['walk-1.json']);
    expect(body.recordings[0]?.bytes).toBe(session.length);
    expect(Number.isNaN(Date.parse(body.recordings[0]?.modifiedAt ?? ''))).toBe(false);

    const read = await fetch(route(host, '&name=walk-1.json'));
    expect(await read.text()).toBe(session);

    expect((await fetch(route(host, '&name=walk-1.json'), {method: 'DELETE'})).status).toBe(200);
    expect((await (await fetch(route(host))).json() as {recordings: unknown[]}).recordings).toEqual([]);
  });

  it('shares one directory between applications, whatever port each serves on', async () => {
    const camera = await start({app: 'camera-app', title: 'Camera App', port: await freePort()});
    const fusion = await start({app: 'fusion-app', title: 'Fusion App', port: await freePort()});
    const session = JSON.stringify({schema: 'twrmc/pose-3d-session', version: 1, events: [1]});

    await fetch(route(camera, '&name=from-camera.json'), {method: 'PUT', body: session});
  ...
  it('refuses a name that is not a recording, a body that is not JSON, and an unknown method', async () => {
    ...
    expect((await fetch(route(host, '&name=walk.json'), {method: 'PUT', body: 'not json'})).status).toBe(400);
    expect((await fetch(route(host, '&name=walk.json'), {method: 'PATCH', body: '{}'})).status).toBe(405);
  ...
  it('lists nothing before anything has been recorded, and skips files that are not recordings', async () => {
    ...
    const body = (await (await fetch(route(host))).json()) as {recordings: Array<{name: string}>};
