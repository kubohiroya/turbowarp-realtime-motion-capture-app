...
describe('acquireRunLock', () => {
  ...
  it('refuses to start a second instance of the same application', async () => {
    ...
    expect(second.acquired === false && second.holder).toMatchObject({
      pid: 100,
      app: 'camera-app',
    });
  });
  ...
  it('replaces a lock that is missing a field', async () => {
    await writeRawLock(
      'camera-app',
      JSON.stringify({ app: 'camera-app', pid: 100 }),
    );
    alive.add(200);
...
describe('release', () => {
  ...
  it('leaves a lock another host has taken over', async () => {
    ...
    const record = JSON.parse(
      await readFile(join(directory, 'camera-app.lock'), 'utf8'),
    ) as RunLockRecord;
...
describe('findPortHolder', () => {
  it('names the other application holding the port', async () => {
    ...
    expect(holder.kind !== 'unknown' && holder.record).toMatchObject({
      app: 'fusion-app',
      pid: 300,
    });
  });
