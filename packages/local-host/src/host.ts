...
export type LocalHostFailure =
  ...
  | {readonly reason: 'port-held-by-application'; readonly holder: RunLockRecord}
...
export async function startLocalHost(options: LocalHostOptions): Promise<LocalHostResult> {
  ...
  if (!acquired.acquired) {
    return {started: false, reason: 'already-running', holder: acquired.holder};
  }
  ...
  try {
    ...
    const holder = await findPortHolder({...lockEnvironment, port: options.port, app: options.app});
    if (holder.kind === 'application') {
      return {started: false, reason: 'port-held-by-application', holder: holder.record};
    }
...
async function serve(
  ...
  host = await createLoopbackPreviewHost({
     ...
     */
    onConnect: () => {
      host?.emit('dsl', published === null ? {state: 'none'} : {state: 'published', ...published});
    },
    ...(options.onError === undefined ? {} : {onError: options.onError})
  });

  const watcher = options.dsl === undefined ? null : startWatcher(options.dsl, options, (next) => {
    published = next;
    host?.emit('dsl', {state: 'published', ...next});
  });
  await watcher?.publishNow().catch((error: unknown) => options.onError?.(error));
  const rechecks = watcher === null ? null : scheduleStartupRechecks(watcher, options);
...
function scheduleStartupRechecks(
  ...
  const delays = options.startupRecheckDelaysMs ?? defaultStartupRecheckDelaysMs;
  const timers = delays.map((delayMs) =>
    setTimeout(() => {
      void watcher.publishNow().catch((error: unknown) => options.onError?.(error));
    }, delayMs).unref()
  );
  return {
    cancel: () => {
...
function startWatcher(
  ...
  const watcher = createStableSourceWatcher<string, string, string>({
    ...
    onPublication: (publication) => {
      onPublication({
        name: publication.sourcePath.split('/').at(-1) ?? publication.sourcePath,
