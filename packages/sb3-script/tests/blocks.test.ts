...
describe('fields', () => {
  ...
  it('keeps the stable id of a named Scratch entity', () => {
    const selection = {name: 'selected camera', id: 'variable:selected-camera'};
    const blocks = buildBlocks([
      script({x: 0, y: 0}, [
        block('data_setvariableto', {VALUE: text('front')}, {VARIABLE: field(selection)})
      ])
    ]);
...
describe('control flow', () => {
  it('links nested substacks to their owning C blocks', () => {
      ...
      script({x: 0, y: 0}, [
        ...
        forever([
          ifElse(
            ...
            [block('shell_hideAppLoading'), repeat(2, [block('looks_nextcostume')])],
    ...
    const foreverBlock = blocks['s1b2'] as {inputs: Record<string, unknown[]>};
    ...
    expect(blocks[ifId]).toMatchObject({opcode: 'control_if_else', parent: 's1b2'});
    expect(blocks[successId]).toMatchObject({opcode: 'shell_hideAppLoading', parent: ifId});
    expect(blocks[failureId]).toMatchObject({opcode: 'shell_showAppError', parent: ifId});
    expect(blocks[blocks[successId]?.next as string]).toMatchObject({opcode: 'control_repeat'});
  });
      ...
      script({x: 0, y: 0}, [
        ...
        ifElse(
          ...
          [
            ...
            ifThen(block('camera_isCameraRunning'), [block('camera_showCameraPreview')])
          ]
        ),
  ...
  it('accepts reporter inputs in comparison conditions', () => {
    const blocks = buildBlocks([
      script({x: 0, y: 0}, [
        ifThen(greaterThan(variable(namedReference('count', 'variable:count')), number(0)), [
          block('shell_hideAppLoading')
        ])
      ])
    ]);
  ...
  it('does not renumber another script when one script changes', () => {
    const secondScript = script({x: 0, y: 200}, [whenFlagClicked(), block('shell_showAppNotice')]);
...
describe('stable Scratch references', () => {
  ...
  it('encodes variable, list, and broadcast inputs with their ids', () => {
    expect(variable(qrText).encode({block: () => '', stack: () => ''})).toEqual([
      3,
      [12, 'qr text', 'variable:qr-text'],
      [10, '']
    ]);
    ...
    expect(broadcast(scanQr).encode({block: () => '', stack: () => ''})).toEqual([
      1,
  ...
  it('builds a representative QR pairing accumulation flow', () => {
    ...
    expect(blocks['s1b2']?.fields['VARIABLE']).toEqual(['qr text', 'variable:qr-text']);
    expect(blocks['s1b5']?.fields['LIST']).toEqual(['qr parts', 'list:qr-parts']);
    expect(Object.values(blocks).map(({opcode}) => opcode)).toEqual(
      expect.arrayContaining([
