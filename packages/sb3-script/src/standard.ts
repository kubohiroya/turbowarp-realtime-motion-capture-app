...
export const whenBroadcastReceived = (message: NamedReference): BlockNode =>
  block(
    ...
    {},
    { BROADCAST_OPTION: field(message) },
  );
...
export const repeat = (
  times: number | InputValue,
  body: readonly BlockNode[],
): BlockNode =>
  block('control_repeat', {
    TIMES: numericInput(times),
    SUBSTACK: substack(body),
  });
...
export const stop = (
  option: 'all' | 'this script' | 'other scripts in sprite',
): BlockNode => block('control_stop', {}, { STOP_OPTION: option });
...
export const setVariable = (
  ...
  block('data_setvariableto', { VALUE: value }, { VARIABLE: field(target) });
