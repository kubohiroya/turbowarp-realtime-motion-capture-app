...
レンズ校正を実装しています。段階3では、全カメラの2D姿勢推定と、その計測を実装しています。段階4では、1ページ内での空間と時刻の校正を実装しています。段階5では、3Dサービス（#34）への接続を実装しています。あわせて、カメラなしで動かすための録画と再生（DEBUG_CAMERA_REPLAY）を実装しています。
...
フラグ`debugCameraReplayV1`（`packages/app-shell/src/apps/local.ts`の`appFlags`）が有効なビルドでだけ、
メニューに出ます。既定では無効です。
