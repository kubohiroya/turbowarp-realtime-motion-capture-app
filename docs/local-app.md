...
`scripts/app-scripts/avatar.ts`で、アバターの割り当て、立ち位置、向き、演出DSLのエフェクトもfusion appと
同じです（[3Dサービス](pose-3d-service.md#アバターへの出力)）。local-appには`turbowarp-aframe`と
`turbowarp-yaml-json`を加え、起動時フラグ`avatarRetargetV1`と`protocolV1Codec`（演出DSLの検証）を
有効にしました。
...
| 演出ファイルを選ぶ       | 演出DSL（YAMLかJSON）を選ぶ。正しいものだけを使い、演者のエフェクトをアバターの表情で表す |

アバターの場面は背景が透明で、カメラの並びの上に重ねて描きます。

演出DSLは、演者をDSLの並び順で枠に対応させ（1人目が`slot-1`）、アバターが認識されるとその演者の
`recognitionStartEffect`を1・`recognitionEndEffect`を0に、見失うと逆にします。デモのDSLは
`apps/fusion-app/avatars/demo-performance.yaml`です（デモのアバターの`happy`と`glow`を使う）。
...
- ブラウザ（演出DSL）：同じ計測ビルドで、「演出ファイルを選ぶ」からデモのDSLを保存して開くと
  「演出ファイルを使います。演者: 2人」と表示された。合成録画の再生中、`slot-1`が認識されると`happy`＝1・
  `glow`＝0、`slot-2`が認識されると`glow`＝1・`happy`＝0が設定され、見失うと逆になった。
  `protocolV1Codec`を有効にする前は、DSLの検証のblockが無いため「演出ファイルが正しくありません:
  undefined」になっていた。
