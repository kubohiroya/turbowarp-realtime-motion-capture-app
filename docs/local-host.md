...
ポーズの録画（`twrmc/pose-3d-session`）を、会場のPCのディレクトリに置きます。アプリごとにポートが
違ってもディレクトリは同じなので、camera appで録った録画をfusion appで再生できます。

録画は、録りながら1行ずつ**圧縮ストリームへ**書き足します。ローカルホストは録画ごとにgzipのストリームを
開いたままにし、届いた行をそこへ流して、1回ごとにフラッシュします。

- ページが持つのは書き足す前の数行だけなので、長い録画が録れる
- ディスクには最初から圧縮した大きさ（生の3分の1ほど）しか置かれない
- 録っている最中でも、途中で落ちた後でも、フラッシュ済みのところまで読める（展開は読む側が行う）
- 終わらせるのはストリームを閉じて名前を付け替えるだけなので、一瞬で済む

| メソッド                                             | 内容                                                                                                                            |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| `GET /recordings`                                    | 一覧（名前、バイト数、更新時刻）                                                                                                |
| `GET /recordings?name=<名前>`                        | 保存したままのバイト列をディスクから流す（`.gz`は`application/gzip`。展開は読む側。録画中や途中で切れたものも、そこまで読める） |
| `POST /recordings?name=<名前>&mode=start`            | 録画を始める（名前は`.jsonl.gz`。本文はヘッダ行。同じ名前の内容は消える）                                                       |
| `POST /recordings?name=<名前>&mode=append`           | 行を書き足す（ディスク上で512 MBまで）                                                                                          |
| `POST /recordings?name=<名前>&mode=finish&to=<名前>` | ストリームを閉じ、`<名前>.jsonl.gz`に付け替える                                                                                 |
| `PUT /recordings?name=<名前>`                        | 圧縮しない録画を一度に書き込む（`.json`はJSON、`.jsonl`は行として検査、512 MBまで）                                             |
| `DELETE /recordings?name=<名前>`                     | 削除（録画中なら、ストリームを閉じてから）                                                                                      |

上限はディスク上の大きさ（圧縮後）で数えます。会場のディスクに置かれ、運用者が片付けることになるのは
その大きさだからです。放っておかれた録画（ページが閉じられたもの）は、10分触られなければ閉じ、ローカル
ホストを止めるときにも閉じます。閉じればgzipの終端が書かれるので、どのgzipの読み手でもそのまま読めます。

`mode=start`と`mode=append`は、行がJSONのオブジェクトで`type`を持つことを確かめます。録画でないものを
書き込めば、再生のときではなく書き込むときに分かります。

名前は英数字と`.`・`-`・`_`で、`.jsonl.gz`・`.jsonl`・`.json`で終わるものだけを受け付けます。区切り文字も
先頭のドットも受け付けないので、ディレクトリの外は指せません。ほかの経路と同じくトークンが要ります。
