...
録画は、録りながら1行ずつ書き足し、終わったところで圧縮します。ページが持つのは数行だけなので長い録画が
録れ、途中で落ちてもそこまでが残り、残るファイルは6分の1ほどの大きさになります。

| メソッド                                             | 内容                                                                            |
| ---------------------------------------------------- | ------------------------------------------------------------------------------- |
| `GET /recordings`                                    | 一覧（名前、バイト数、更新時刻）                                                |
| `GET /recordings?name=<名前>`                        | 読み出し（`.gz`は`Content-Encoding: gzip`で返すので、読む側は行がそのまま届く） |
| `POST /recordings?name=<名前>&mode=start`            | 録画を始める（本文はヘッダ行。同じ名前の内容は消える）                          |
| `POST /recordings?name=<名前>&mode=append`           | 行を書き足す（合計64 MBまで）                                                   |
| `POST /recordings?name=<名前>&mode=finish&to=<名前>` | `.jsonl.gz`へ圧縮して保存し、書きかけを消す                                     |
| `PUT /recordings?name=<名前>`                        | 一度に書き込む（`.json`はJSON、`.jsonl`は行として検査、64 MBまで）              |
