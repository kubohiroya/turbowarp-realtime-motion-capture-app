...
2Dの人物を対にして読みます。fusion-v0の`personId`は「最初に見たカメラと追跡ID」なので、追跡が切れると
変わり（時間方向の同定は段階4）、それと同じIDの2Dを出すものもありませんでした。そこでサービスが
...
- **枠**：現れた人物は空いている最も小さい番号の枠を取り、見えている間は保ち、`HOLD_MS`見えなければ
  ...
  段階4までは、追跡が切れるとアバターが別の人へ移ることがあります。
- **2D**：3Dの`personId`が指すカメラと追跡IDの人物を、そのカメラの最新の2Dフレームから取ります。サービスが
