...
- 今のカメラに合うレンズ校正が適用されている（`calibrationId`にそのprofile IDを入れるため）。
  「合う」は、Camera Sourceが今のカメラの設定（解像度、ズーム、フォーカス）と校正時の条件を比べた結果
  （`camera profile compatibility`が`compatible`）で判断する。合わなければ理由を添えて`LENS_INCOMPATIBLE`を
  表示し、始めない
...
実行中も、推論の前に毎回レンズ校正がカメラに合っているかを確かめます。フォーカスやズームが変わったり、
カメラが別の解像度で開き直されたりして合わなくなったら、推論を止めて`POSE_LENS_INCOMPATIBLE`を理由と
ともに表示します。合わない校正のIDを付けたフレームを送り続けないためです（統合アプリの3Dサービスも、
カメラモデルと違う`calibrationId`のフレームは拒否します）。カメラの設定を戻せば、また開始できます。
...
- ブラウザ（レンズ校正の再確認、#8）：camera appのペアリング、カメラ、レンズ校正の適合、推論のblockを
  置き換え、メニューから次を確認した。
  - 適合が`incompatible`のときは、`LENS_INCOMPATIBLE`と理由を表示して推論を始めない
  - 30 FPSで推論中に`incompatible`に変わると、次の推論の前に止まり、`POSE_LENS_INCOMPATIBLE`と理由を表示する
  - `compatible`に戻せば再び開始でき、確認を加えてもFPSは30のまま
